<div class="min-h-screen flex items-center justify-center bg-bg px-4">
	<div class="w-full max-w-sm mx-auto">
		<!-- 로고 및 소개 -->
		<div class="text-center mb-12">
			<div class="w-16 h-16 mx-auto mb-5 rounded-2xl bg-primary-bg flex items-center justify-center">
				<span class="text-3xl text-primary font-serif font-bold">Q</span>
			</div>
			<h1 class="text-2xl font-bold text-text tracking-tight">DailyQT</h1>
			<p class="mt-2 text-text-secondary text-sm leading-relaxed">매일 말씀과 함께하는<br/>나만의 묵상 시간</p>
		</div>

		<!-- 기능 미리보기 -->
		<div class="space-y-3 mb-10">
			{#each [
				{ icon: '&#x1F4D6;', text: '맞춤형 성경 본문과 묵상 가이드' },
				{ icon: '&#x270D;', text: '해설, 질문, 기도 안내' },
				{ icon: '&#x1F4CA;', text: '묵상 기록과 스트릭 관리' },
			] as item}
				<div class="flex items-center gap-3 px-4 py-3 bg-surface rounded-xl border border-border">
					<span class="text-lg">{@html item.icon}</span>
					<span class="text-sm text-text">{item.text}</span>
				</div>
			{/each}
		</div>

		<!-- 로그인 버튼 -->
		<a
			href="/auth/google"
			class="flex items-center justify-center gap-3 w-full px-6 py-3.5 bg-surface border border-border rounded-2xl shadow-sm hover:shadow-md hover:border-primary/30 transition-all text-text font-medium"
		>
			<svg class="w-5 h-5" viewBox="0 0 24 24">
				<path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4" />
				<path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
				<path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" />
				<path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" />
			</svg>
			Google로 시작하기
		</a>

		<p class="text-center text-xs text-text-secondary mt-4">
			로그인 시 <a href="/privacy" class="underline hover:text-primary">개인정보처리방침</a>에 동의합니다.
		</p>
	</div>
</div>
